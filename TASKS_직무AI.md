...
**전체 자동 검증: 164 / 164 통과** (Edge headless, 2026-08-13)
...
### 3. 로그인을 선택으로 (2026-08-13)

**요청**: "그냥 로그인은 필수로 두지 말고 선택사항으로 하자"

PRD 2·6·8항이 모두 "로그인 필수"를 전제하고 있어 먼저 알리고 범위를 확인했다.
**경계를 로그인 여부가 아니라 "무엇이 사내 정보인가"로 다시 그었다.**

| 대상 | 로그인 | 이유 |
|---|---|---|
| 법령·고시·판례·행정해석·HR 이슈·수집 시각 | 불필요 | 법제처·대법원·언론의 공개 자료. 요구할 근거가 없고 아침 5분에 마찰만 더한다 |
| 담당자 노트 · 큐레이션 코멘트 | 필요 | 사내 정보. PRD 2항이 로그인을 요구한 이유가 이것이다 |

`labor_items` · `labor_runs` 의 읽기 정책을 `anon` 에도 열고, 노트·큐레이션은
`labor_members` 등록자로 그대로 뒀다.

**화면**

- 게이트가 첫 화면이 아니다. 필요할 때 여는 오버레이이며 **닫고 나갈 길**을 뒀다
  (닫기 버튼 · 바깥 클릭 · Esc). 로그인이 필수가 아니므로 막다른 길을 만들지 않는다
- 헤더는 `담당자 로그인` ↔ `로그아웃 + 계정 배지` 로 전환된다
- 비로그인 노트 카드에는 건수를 **`0건` 이 아니라 `로그인 필요`** 로 적는다 —
  못 보는 것과 없는 것은 다르다
- 로그아웃은 화면을 닫지 않는다. 노트만 비우고 브리핑은 다시 받아 그린다

| 검증 항목 | 결과 |
|---|---|
| 로그인 없이 브리핑 12건 + HR 이슈 표시 | PASS |
| 브리핑은 비로그인 직접 호출로도 받음 | PASS |
| **노트·큐레이션은 비로그인 직접 호출 0건** | PASS |
| 비로그인엔 로그인 안내 · 건수를 `0건` 이라 적지 않음 | PASS |
| 게이트를 닫고 브리핑으로 복귀 | PASS |
| 로그인 후 게이트 닫힘 · 버튼 전환 | PASS |
| 로그아웃 시 노트만 닫히고 브리핑은 남음 | PASS |
| 기존 권한 검증(팀원 쓰기 403 · 작성자 아닌 삭제 거부) 유지 | PASS |
